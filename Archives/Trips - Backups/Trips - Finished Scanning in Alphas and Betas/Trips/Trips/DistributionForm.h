...
static std::vector<std::vector<double> > alphaLightMatrix;
static std::vector<std::vector<double> > alphaMediumMatrix;
static std::vector<std::vector<double> > alphaHeavyMatrix;

static std::vector<double> betaLightVector;
static std::vector<double> betaMediumVector;
static std::vector<double> betaHeavyVector;
...
private: System::Void loadL_Click(System::Object^  sender, System::EventArgs^  e) {
			 OpenFileDialog^ openFileDialog1 = gcnew OpenFileDialog;
				...
				if(lightMatrix[0].size() != (X[0].size()+1) ) {
...
private: System::Void loadM_Click(System::Object^  sender, System::EventArgs^  e) {
			 OpenFileDialog^ openFileDialog1 = gcnew OpenFileDialog;
				...
				if(mediumMatrix[0].size() != (X[0].size()+1) ) {
					MessageBox::Show("Invalid Column Dimensions. Try different file or fix it.", "Notification");
					mediumMatrix.clear();
					return;
					}
...
private: System::Void loadH_Click(System::Object^  sender, System::EventArgs^  e) {
			 OpenFileDialog^ openFileDialog1 = gcnew OpenFileDialog;
				...
				if(heavyMatrix[0].size() != (X[0].size()+1) ) {
					MessageBox::Show("Invalid Column Dimensions. Try different file or fix it.", "Notification");
					heavyMatrix.clear();
					return;
					}
